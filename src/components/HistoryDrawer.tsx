...
interface HistoryDrawerProps {
  ...
  onExportSingle?: (record: HistoryRecord) => void;
  onExportAll?: () => void;
}
...
export function HistoryDrawer({ open, onClose, onSelect, onExportSingle, onExportAll }: HistoryDrawerProps) {
  ...
  const handleExportSingle = (e: React.MouseEvent, r: HistoryRecord) => {
    e.stopPropagation();
    onExportSingle?.(r);
    onClose();
  };
      ...
      {open && (
        <>
          ...
          >
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
                          ...
                          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              type="button"
                              onClick={(e) => handleExportSingle(e, r)}
                              className="w-6 h-6 rounded-md hover:bg-bg-soft flex items-center justify-center text-text-3 hover:text-duo"
                              aria-label="导出 PDF"
                              title="导出此条 PDF"
                            >
                            ...
                            <button
                              ...
                              className="w-6 h-6 rounded-md hover:bg-bg-soft flex items-center justify-center text-text-3 hover:text-[var(--color-refuted-text)]"
                              aria-label="删除"
                            >
            ...
            {/* 底部 */}
            {records.length > 0 && (
              <div className="px-5 py-3 border-t-2 border-[var(--color-separator)] space-y-2.5">
                {/* 批量导出按钮 */}
                {onExportAll && filtered.length > 0 && (
                  <AppleButton
                    variant="primary"
                    size="md"
                    fullWidth
                  ...
                  >
                    📄 导出 {filter === 'all' ? '全部' : '当前筛选'} {filtered.length} 条为 PDF
                  </AppleButton>
                )}
